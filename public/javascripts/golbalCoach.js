...
$(document).ready(function() {
    ...
    // Username link click
    $('#ListCoach table tbody').on('click', 'td a.linkshowuser', showUserInfo);

    // Add User button click
    $('#btnCoachUser').on('click', addUser);
    //buton limipiar
    $('#btnlimpiar').on('click', formReset);

    // Delete User link click
    $('#ListCoach table tbody').on('click', 'td a.linkdeleteuser', deleteUser);

    $('#ListCoach table tbody').on('click', 'td a.linkupdateuser', ModificaUser);

    $('#ListCoach table tbody').on('click', 'td a.linkupuser', UpdateUser);
...
function populateTable() {
    ...
    $.getJSON( '/users/ListCoach', function( data ) {
$('#ListCoach table tbody').on('click', 'td a.linkshowuser', showUserInfo);
        ...
        $('#ListCoach table tbody').html(tableContent);
    });
};
...
function addUser(event) {
    ...
    var errorCount = 0;
    $('#addCoach input').each(function(index, val) {
    ...
    if(errorCount === 0) {
        ...
        var newUser = {
            'email': $('#addCoach fieldset input#inputUserEmail').val(),
            'nombre': $('#addCoach fieldset input#inputUserNombres').val(),
            'apellido': $('#addCoach fieldset input#inputUserApellidos').val(),
            'celular': $('#addCoach fieldset input#inputUserCelular').val()
...
function deleteUser(event) {
    ...
    if (confirmation === true) {
        ...
        $.ajax({
            type: 'DELETE',
            url: '/users/deleteCoach/' + $(this).attr('rel')
        }).done(function( response ) {
...
function UpdateUser(event) {
    ...
    if(errorCount === 0) {
        ...
        $.ajax({
            type: 'POST',
            data: updateUser,
            url: '/users/updateCoach/'+$(this).attr('rel'),
